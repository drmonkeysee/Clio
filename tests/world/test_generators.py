...
def generator(request: pytest.FixtureRequest) -> WorldGenerator:
    return request.param


def test_output_shapes(generator: WorldGenerator) -> None:
...
def test_output_dtypes(generator: WorldGenerator) -> None:
    world = generator(ROWS, COLS, seed=SEED)
...
def test_float_fields_in_unit_range(generator: WorldGenerator) -> None:
    world = generator(ROWS, COLS, seed=SEED)
...
def test_terrain_values_are_valid_terrain_types(generator: WorldGenerator) -> None:
    world = generator(ROWS, COLS, seed=SEED)
...
def test_biome_equals_terrain_but_is_distinct_array(generator: WorldGenerator) -> None:
    world = generator(ROWS, COLS, seed=SEED)
    assert np.array_equal(world.biome, world.terrain)
    assert world.biome is not world.terrain


def test_determinism(generator: WorldGenerator) -> None:
...
def test_different_seeds_produce_different_results(generator: WorldGenerator) -> None:
